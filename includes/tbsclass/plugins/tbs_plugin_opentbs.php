...
class clsOpenTBS extends clsTbsZip
{
	 ...
	 * ExtInfo will be an array with keys 'load', 'br', 'ctype' and 'pic_path'. Keys 'rpl_what' and 'rpl_with' are
	 * optional. load:     files in the archive to be automatically loaded by OpenTBS when the archive is loaded.
	 * Separate files with comma ';'. br:       string that replace break-lines in the values merged by TBS, set to
	 * false if no conversion. frm:      format of the file ('odf' or 'openxml'), for now it is used only to activate a
	 * special feature for openxml files ctype:    (optional) the Content-Type header name that should be use for HTTP
	 * download. Omit or set to '' if not specified. pic_path: (optional) the folder nale in the archive where to place
	 * pictures rpl_what: (optional) string to replace automatically in the files when they are loaded. Can be a string
	 * or an array. rpl_with: (optional) to be used with 'rpl_what',  Can be a string or an array. User can define his
	 * own Extension Information, they are taken in acount if saved int the global variable $_OPENTBS_AutoExt.
	 */
	...
	/**
	 * Prevent from the problem of missing spaces when calling ->MsWord_CleanRsID() or under certain merging
	 * circumstances. Replace attribute xml:space="preserve" used in <w:t>, with the same attribute in <w:document>.
	 ...
	 * @param       int      [$Offset] Since a DCX can have several sections, and each section can have its own
	 *                                 header/footer, this options
	 *
	 ...
	 * In Docx, section-breaks <w:sectPr> can be saved in the last <w:p> of the section, or just after the last <w:p>
	 * of the section. In practice, there is always at least one sectin-break and only the last section-break is saved
	 * outside the <w:p>.
	 */
	...
	*/
	 ...
	 * The file is delete from the declaration file [Content_Types].xml and from the relationships of the specified
	 * files.
	 *
	 * @param  {string} $FullPath The full path of the file to delete.
	 * @param  {array}  $$RelatedTo List of the the full paths of the files than may have relationship with the file to
	 *                              delete.
	 *
	 * @return {mixed} False if it is not possible to delete the file, or the number of modifier relations ship in case
	 *                 of success (may be 0).
	 */
	...
	/**
	 * Return an object that represents the informations of an .rels file, but for optimization, targets are scanned
	 * only for asked directories. The result is stored in a cache so that a second call will not compute again. The
	 * function stores Rids of files existing in a the $TargetPrefix directory of the archive (image, ...).
	 *
	 ...
	 * Take car that there is another technic for listing and adding targets wish is working with a persistent object
	 * which is commit at the end of the merge..
	 *
...
If they are blank spaces, line beaks, or other unexpected characters, then you have to check your code in order to avoid them.";
	 ...
	 * @param {string|false} $caller A text describing the calling function, for error reporting purpose. If
	 *                               caller=false it means TbsStoreLoad().
	 */
	 ...
	 * @param {boolean} $OnlyInner Set to true to keep the content inside the element. Set to false to delete the
	 *                             entire element. Default is false.
	 */
...
var $Parent = false;
	...
	var $SelfClosing;
		...
		var $Txt; // start tag: position of the end

	var $pET_PosBeg = false;    // start tag: source

	var $pST_PosEnd = false; // end tag: position of the begining

	var $pST_Src = false; // parent object
	 ...
	 *  of the source of the parent element. The parent element can receive the changes of the object using method
	 *  UpdateParent().
	 */
