...
var $imageName;
  ...
  var $repeaterFunction = "";
    ...
    var $scriptFileName = "";   //should be ASC or DESC
  ...
  <ul>
    ...
    foreach ($this->columnSet as $val) {
      if ($val->isDatabaseField) {
        $strResult .= "
    <li><div class=\"checkbox no-margin\"><label><input class=checkbox-inline type=\"checkbox\" name=\"dataHeader" . $val->fieldName . "\" id=\"dataHeader" . $val->fieldName . "\" checked onClick=\"javascript:showHideField($counter, '" . $val->fieldName . "')\" />" . $val->columnName . "</label></div></li>";
      }
      if ($val->intColSpan <= 0) {
        $counter++;
      }
