      ...
      case 'radio' :
        if ($obj['visible']) {
          if ($this->readonly) {
            ...
            foreach ($obj['values'] as $key => $opsi) {
              $counterId++;
              $strResult .= $strPreResult . " id=\"" . $obj['name'] . $counterId. "\" value='" . $this->_escapeString($opsi['value']) . "'";
      ...
      if ($i < $colCount) {
        ...
        ($j < (($i + 1) * $jmlBaris) && $j < count($this->formObject[$tabPageNumber][$groupNumber])); $j++
          ...
          if (!($this->formObject[$tabPageNumber][$groupNumber][$j]['renderLabel'])) {
            $strResult .= "<div class=\"col-sm-12\">".$this->_printObject($this->formObject[$tabPageNumber][$groupNumber][$j])."</div></div>";
          } else {
