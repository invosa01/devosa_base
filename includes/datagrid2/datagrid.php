...
<div class='contextMenu' id=\"divContext" . $this->gridName . "\">
  ...
  //Fungsi Tambahan Untuk GetCriteria
    ...
    function getDataTypeGroup($dataType = null){
        $groupType = null;
        if (!empty($dataType)){
            $groupInteger = array('bigint','bigserial','double precision','integer','numeric','real','money','smallint','smallserial','serial');
            $groupString = array('bit','bit varying','character','character varying','text','xml');
            $groupDate = array('date','time with time zone','time without time zone','timestamp with time zone','timestamp without time zone');
            $groupBoolean = array('boolean');
            ...
            if ($this->pageSearchCriteria!="")
                        ...
                        if ($col->fieldName!='' && $col->searchable)
                        {
                            ...
                            if ($strResult == ""){
                                if (!empty($groupType)){
                                    if ($groupType == 'integer' || $groupType == 'date' || $groupType == 'boolean'){
                                        if (is_integer($this->pageSearchCriteria) || is_numeric($this->pageSearchCriteria)){
                                            $strResult = $this->_formatFieldName($col->fieldName) . " = '" . $this->pageSearchCriteria . "'";
                                        }
                                ...
                                if (!empty($groupType)){
                                    if ($groupType == 'integer' || $groupType == 'date' || $groupType == 'boolean'){
                                        if (is_integer($this->pageSearchCriteria) || is_numeric($this->pageSearchCriteria)){
                                            $strResult .= " OR " . $this->_formatFieldName($col->fieldName) . " = '" . $this->pageSearchCriteria . "'";
                                        }
                                    }else if($groupType == 'string'){
                ...
                else
                {
                    ...
                    if (!empty($groupType)){
                        if ($groupType == 'integer' || $groupType == 'date' || $groupType == 'boolean'){
                            if (is_integer($this->pageSearchCriteria) || is_numeric($this->pageSearchCriteria)){
                                $strResult = $this->_formatFieldName($this->pageSearchBy) . " = '" . $this->pageSearchCriteria . "'";
                            }
                        }else if($groupType == 'string'){
