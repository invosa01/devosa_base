...
(function($) {
    ...
    function buildNode($node, $appendTo, level, index, opts) {
        ...
        //tambahan buat garis diatasnya
        //if (level > 1){
         /*   var $UpLineRow = $("<tr/>").addClass("lines").addClass($node.attr("class"));
            var $UpLeft = $("<td>").addClass("lvl left");
            var $upRight = $("<td>").addClass("lvl right");
            $UpLineRow.append($UpLeft).append($upRight);
            $tbody.append($UpLineRow);
        */
            var $lvltable = $("<table cellpadding='0' cellspacing='0' border='0'/>");
            var $lvltbody = $("<tbody/>");
            var $lvlLineRow = $("<tr/>").addClass("lines").addClass($node.attr("class"));
            var $UpLeft = $("<td>").addClass("lvl left");
            var $upRight = $("<td>").addClass("lvl right");
            $lvlLineRow.append($UpLeft).append($upRight);
            $lvltbody.append($lvlLineRow);
            $lvltable.append($lvltbody);

            var $lvlLineRow = $("<tr/>").addClass("lines").addClass($node.attr("class"));
            var $updata = $("<td>").attr("colspan", 2);
            if ($childNodes.length > 1) {
                $updata.attr("colspan", $childNodes.length*2);
            }
            $updata.append($lvltable);
            $lvlLineRow.append($updata);
            $tbody.append($lvlLineRow);
        //}
        // end of tambahan
        ...
        if ($childNodes.length > 0) {
            if (opts.depth == -1 || level+1 < opts.depth) {
                ...
                //var $linesRow = $("<tr/>").addClass("lines v" + level ).addClass($node.attr("class"));
                //$linesRow.addClass($node.attr("class"));
