  ...
  if ($db->connect()) {
    ...
    */
    ...
    if ($strDataUserRole == ROLE_EMPLOYEE or $strDataUserRole == ROLE_SUPERVISOR) {
      //uddin date thru untuk user ESS dibatasi sampai awal bulan
      $strDateBatasYear = date("Y");
      $strDateBatasMonth = date("m");
      $strdateBatas = $strDateBatasYear . "-" . $strDateBatasMonth . "-01";
      if ($strDateThru > $strdateBatas) {
        $strDateThru = $strdateBatas;
        $strDateThru = date('Y-m-d', strtotime('last day of previous month'));
      }
      if ($arrUserInfo["functional_code"] != "") {
        ...
        $strFunctionalcode = "('INISIALINVOSAFUNCTION'"; // inisial masukkan kode functional diri sendiri
