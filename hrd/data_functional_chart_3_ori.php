...
$strSQL .= " WHERE functional_code = 'BOD' ORDER BY weight ASC";//hapus bila sudah ada data sebenar nya
//$strSQL .= " ORDER BY weight ASC";//hapus bila sudah ada data sebenar nya
...
echo "<li><a href=\"?mgt=true\">" . getFunctionalName($strFunc) . "</a>";
while ($row = $db->fetchrow($result)) {
...
function funcChart($strFunc, $n, $orgLvl)
{
    ...
    if (hasMember($strFunc)) {
        ...
        $strSQL .= " WHERE t1.head_code = '" . $strFunc . "' $strKriteria";
        if ($n == $pLevel and $pLevel == 1 and $pFunc != "") {
            $strSQL .= " AND t1.functional_code = '" . $pFunc . "' ";
        }
        $strSQL .= "ORDER BY weight ASC";
        echo $strSQL;
        //die();
        $result = $db->execute($strSQL);
        while ($row = $db->fetchrow($result)) {
...
function getFunctionalName($strFunc)
{
    ...
    $strSQL .= " WHERE functional_code = '" . $strFunc . "' ORDER BY weight ASC";
    $result = $db->execute($strSQL);
    while ($row = $db->fetchrow($result)) {
