...
function saveData($db, &$strDataID, &$strError)
{
  ...
  if ($strDataDateThru < $strDataDateFrom) {
    $strError = $error['invalid_date'];
    $bolOK = false;
  }
  ...
  if ($strDataDuration >= 1 and ($strDataType == "AKM"
          ...
          or $strDataType == "KG"
          or $strDataType == "KKM"
          or $strDataType == "KM" //KM oleh sanusi
          or $strDataType == "KSM" //KSM oleh sanusi
          or $strDataType == "LH"
          or $strDataType == "MK"
          or $strDataType == "UMR"
  ...
  // KM,KKM,SD,TD tidak boleh tangal masa depan
  if ($strDataDuration >= 1 and ($strDataType == "KM"
          or $strDataType == "KKM"
          or $strDataType == "KSM" //KSM oleh sanusi
          or $strDataType == "SD"
          or $strDataType == "TD") and ($strToday < $strDataDateFrom or $strToday < $strDataDateThru)
  ) {
    $bolOK = false;
    $strError .= "Special leave cannot more than current date, ";
  }
  ...
  //uddin
  // tidak boleh lebih dari 3 hari: MK,KM
  //if ($strDataDuration > 3 and ($strDataType=="MK" or $strDataType=="KM")){
  //  $bolOK = false;
  //  $strError .= "Leave type ".$strDataType." cannot more than 3 days, ";
  //}
  // tidak boleh lebih dari 2 hari: AKM,KKM,IM,KA,BA
  //if ($strDataDuration > 2 and ($strDataType=="AKM"
  //                            or $strDataType=="KKM"
  //                            or $strDataType=="IM"
  //                            or $strDataType=="KA"
  //                            or $strDataType=="BA")){
  //   $bolOK = false;
  //  $strError .= "Leave type ".$strDataType." cannot more than 2 days, ";
  //}
  //uddin : jika durasi 0 asumsi ambil absen di hari libur semua
  if ($strDataDuration == 0) {
    $bolOK = false;
    $strError .= "Absence cannot on holiday, ";
  }
  ...
  if ($rowL = $db->fetchrow($resS)) {
    ...
    // uddin: cek durasi leave kecuali beberapa cuti
    if ($strDataDuration > $rowL['leave_weight'] and !($strDataType == "A"
            or $strDataType == "C"
            or $strDataType == "DL"
            or $strDataType == "SD"
            or $strDataType == "TD"
      ...
      $strError .= "Your Leave duration $strDataDuration days ,Leave type " . $strDataType . " cannot more than " . $rowL['leave_weight'] . " days, ";
      $bolOK = false;
    }
  }
  ...
  //asli $strSQL  = "SELECT code, deduct_leave FROM hrd_absence_type ";
  //edit by ade sanusi
  $strSQL = "SELECT code, note, deduct_leave FROM hrd_absence_type ";
  ...
  //201601210857 - ade sanusi: query untuk ambil data note tipe absence
  $strSQL = "SELECT code, note, deduct_leave FROM hrd_absence_type ";
  $strSQL .= "WHERE code = '$strDataType'";
  $resS = $db->execute($strSQL);
  $arrLeave = [];
  ...
  if ($bolOK) { // input OK, tinggal disimpan
    ...
    $strBody .= "Nama: " . getEmployeeName($db, $strIDEmployee) . "\n";
    $strBody .= "Tipe Absen: " . $strDataType . " - ( " . $strCutiNote . " )\n";
    //$strBody .= "Absence Type: ".$strDataType."\n";
    $strBody .= "Tanggal: " . $strDataDateFrom . " Until " . $strDataDateThru . "\n";
    $strBody .= "____________________________________________________________________________________" . "\n";
    $strBody .= "Harap segera APPROVE/DENIED Pengajuan Absen Bawahan Anda melalui Sistem ESS ( http://hr.wanaarthalife.com/devosa-uat/ ). " . "\n";
    ...
    if ($strDataID == "") {
      ...
      if ($bolOK) {
        ...
        $resExec = $db->execute($strSQL);
        $strSubject = getSubjectEmail(0, 'Absence', getEmployeeCode($db, $strIDEmployee));
...
if ($db->connect()) {
  ...
  if ($strIDEmployee != "") {
    ...
    $strLeaveDetail = "&nbsp;<br><strong>Leave Detail</strong><br><table class=\"table\" ><tr><th>Year</th><th>Quota</th><th>Additional Quota</th><th>Holiday</th><th>Taken</th><th>Additional Taken</th><th>Remaining</th></tr>";
    $strLeaveDetail .= "<tr><td>" . $arrCuti['prev']['year'] . "</td><td align=\"center\">" . $arrCuti['prev']['quota'] . "</td><td align=\"center\">" . $arrCuti['curr']['add_quota'] . "</td><td align=\"center\">" . $arrCuti['prev']['holiday'] . "</td><td align=\"center\">" . $arrCuti['prev']['taken'] . "</td><td align=\"center\">" . $arrCuti['curr']['add_taken'] . "</td><td align=\"center\">" . ($arrCuti['prev']['remain'] - $arrCuti['prev']['add_taken'] + $arrCuti['prev']['add_quota']) . "</td></tr>";
    $strLeaveDetail .= "<tr><td>" . $arrCuti['curr']['year'] . "</td><td align=\"center\">" . $arrCuti['curr']['quota'] . "</td><td align=\"center\">" . $arrCuti['curr']['add_quota'] . "</td><td align=\"center\">" . $arrCuti['curr']['holiday'] . "</td><td align=\"center\">" . $arrCuti['curr']['taken'] . "</td><td align=\"center\">" . $arrCuti['curr']['add_taken'] . "</td><td align=\"center\">" . ($arrCuti['curr']['remain'] - $arrCuti['curr']['add_taken'] + $arrCuti['curr']['add_quota']) . "</td></tr></table>";
  } else {
    $strLeaveDetail = "";
  }
  //----- TAMPILKAN DATA ---------
  ...
  $strInputDateFrom = "<input class=\"form-control datepicker\" type=text size=15 maxlength=10 name=dataDateFrom id=dataDateFrom value=\"" . $arrData['dataDateFromNow'] . "\" data-date-format=\"" . $_SESSION['sessionDateSetting']['html_format'] . "\">";
  $strInputDateThru = "<input class=\"form-control datepicker\" type=text size=15 maxlength=10 name=dataDateThru id=dataDateThru value=\"" . $arrData['dataDateThruNow'] . "\" data-date-format=\"" . $_SESSION['sessionDateSetting']['html_format'] . "\">";
  ...
  if ($arrData['dataDoc'] != "") {
    $strInputDoc = "<a href=\"absencedoc/" . $arrData['dataDoc'] . "\" target=\"_blank\"> Klik View Document</a>";
  } else {
    $strInputDoc = "No Attachment";
  }
