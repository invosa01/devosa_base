...
$strWordsFlag = getWords("otm flag");
...
if ($db->connect()) {
    if (isset($_REQUEST['btnStart'])) {
        if ($bolCanEdit) {
            ...
            if ($intID > 0) { // error
                // langsung redirect
                redirectPage("otm_calculation_result.php?dataID=$intID");
                //echo "<script>postToURL('otm_calculation_result.php', {'dataID':'$intID'})</script>";
                exit();
            } else {
                if ($strError != "") {
    ...
    $strDefaultFromDefault = $arrDt[0] . "-" . $arrDt[1] . "-" . $arrDt[2];
    ...
    */
    ...
    /*$strInputFlag = generateCheckBox(
        "dataFlag",
        true,
        "",
        "",
        '<strong>' . $strWordsFlag. '</strong>'
    );*/
    //$strInputFlag = generateHidden("dataFlag",1,"");
