...
$f->addFieldSet(getWords("personal information"),3);
...
$f->addFieldSet(getWords("home address (full)"), 3);
...
$f->addFieldSet(getWords("hometown address"), 3);
...
$f->addFieldSet(getWords("basic information"), 3);
$f->addRadio(
    getWords("sex"),
    "gender",
...
$f->addFieldSet(getWords("identity document"), 3);
$f->addInput(
...
$f->addFieldSet(getWords("birth & marital information"), 3);
$f->addInput(
...
$f->addFieldSet(getWords("others"), 2);
...
$tableHobbies = '<table class="table table-striped table-hover">
<thead>
<tr><th style="width: 50px;">No.</th><th class="center">Hobbies</th>
</thead>
<tbody>
<tr><td>1.</td><td>'.generateInput("hobbies", $arrData['hobbies'], "class=t size=80").'</td></tr>
<tr><td>2.</td><td>'.generateInput("hobbies1", $arrData['hobbies1'], "class=t size=80").'</td></tr>
<tr><td>3.</td><td>'.generateInput("hobbies2", $arrData['hobbies2'], "class=t size=80").'</td></tr>
<tr><td>4.</td><td>'.generateInput("hobbies3", $arrData['hobbies3'], "class=t size=80").'</td></tr>
<tr><td>5.</td><td>'.generateInput("hobbies4", $arrData['hobbies4'], "class=t size=80").'</td></tr>
</tbody>
</table>';
$f->addLiteral(
    "",
...
//
$f->addFieldSet(getWords("your total experience in years"), 3);
$f->addInput(
...
$f->addFieldSet(getWords("job reference"), 3);
$f->addLiteral(
    "",
    "dataJobReference",
...
$f->addFile(getWords("upload photo"), "dataPhoto", getPhoto($arrData['file_photo']), ['size' => 40], "image", false, true, true, "", "<span class=\"col-md-12\">Maximun photo size 500kb</span>");
...
$f->addFieldSet(getWords("language skills entry"), 1);
$f->addLabel(
    "",
    "dataInfoLanguage",
...
$f->addFieldSet(getWords("social activities entry"), 1);
$f->addLiteral(
    "",
...
$f->addFieldSet(getWords("other question entry"), 1);
$f->addLiteral(
    "",
    "dataOtherLiteral",
...
//untuk upload cv
$f->addTabPage(getWords("CV"), 3);
$f->addFieldSet(getWords("cv file upload"), 3);
//$f->addLabel(getWords("instruction"), "CvInstruction", );
$maxFileSize = "<span>".getWords("max filesize") . " 1MB</span>";
if ($arrData['file_cv']) {
    ...
    $f->addFile(getWords("change cv"), "dataCv", "", ['size' => 40], "string", false, true, true, "", $maxFileSize);
} else {
    $f->addFile(getWords("upload cv"), "dataCv", "", ['size' => 40], "string", false, true, true, "", $maxFileSize);
...
$strPageDesc = getWords('candidate entry form');
    ...
    if ($strDataID != "") {
        if ($rowDb = $tblCandidate->findById($strDataID)) {
            if ($isCandidate && $rowDb['candidate_entry_date'] == "") {
                $rowDb['candidate_entry_date'] = date($_SESSION['sessionDateSetting']['php_format']);
            }
            return $rowDb;
        }
    }
...
function getFamily($strDataID)
{
    ...
    $strResult = "
      <table class=\"table table-striped table-hover\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\">
        <thead>
        <tr>
          <th width=150 rowspan=2>" . getWords("family") . "</th>
          <th rowspan=2>" . getWords("name") . "</th>
          <th width=80 rowspan=2>" . getWords("sex") . "</th>
          <th width=250 rowspan=2>" . getWords("birth place") . "</th>
          <th width=120 rowspan=2>" . getWords("date of birth") . "</th>
          <th width=200 rowspan=2>" . getWords("last education") . "</th>
          <th width=150 colspan=2 class='center'>" . getWords("occupation") . "</th>
        </tr>
        <tr>
          <th width=150 class='center'>" . getWords("position") . "</th>
          <th width=150 class='center'>" . getWords("company") . "</th>
        </tr>
        </thead>
        <tbody>";
    ...
    foreach ($arrMainFamily as $mainFamily) {
        ...
        <tr id='mainFamily$counter'>
          ...
          <td nowrap>" . generateInput("familyDOB" . $counter, $row['dob'], "maxlength=10 size='10'") . "</td>
        ...
        <tr id='mainFamily$counter' $strStyle>
          ...
          <td nowrap>" . generateInput("familyDOB" . $counter, "", "maxlength=10 size='10'") . "</td>
    ...
    foreach ($arrOwnFamily as $ownFamily)    //Tidak masuk loop(?)
    {
        ...
        <tr id='mainFamily$counter'>
          ...
          <td nowrap>" . generateInput("familyBirthPlace" . $counter, $row['birthplace'], "size=30") . "</td>
          <td nowrap>" . generateInput("familyDOB" . $counter, $row['dob'], "size='10'") . "</td>
          <td nowrap>" . generateInput("familyEducation" . $counter, $row['education'], "style='width: 100%'") . "</td>
          <td nowrap>" . generateInput("familyPosition" . $counter, $row['position'], "style='width: 100%'") . "</td>
          <td nowrap>" . generateInput("familyCompanyName" . $counter, $row['company_name'], "size='15'") . "</td>
        </tr>";
...
function getLanguageSkill($strDataID)
{
    ...
    $strResult = "
      ...
      <thead>
        <tr>
          ...
          <th class=center width=150>" . getWords("listening") . "</th>
          <th class=center width=150>" . getWords("reading") . "</th>
          <th class=center width=150>" . getWords("speaking") . "</th>
          <th class=center width=150>" . getWords("writing") . "</th>
        </tr>
      </thead>
    ...
    $strResult .= "
        <tfoot>
        <tr>
          <th colspan=6><a href=\"javascript:myClient.showMoreLanguageSkill()\">" . getWords("show more") . "</a></th>
        </tr>
        </tfoot>";
    $strResult .= "
      </table>
...
function getQuestion($db, $strDataID)
{
    ...
    // );
    $strResult = "
      <table width='100%' class=\"table table-striped table-hover\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\">
      <thead>
        <tr>
          <th width=35 rowspan=2>No.</th>
          <th rowspan=2>" . getWords("question") . "</th>
          <th class=center colspan=2>" . getWords("answer") . "</th>
          <th class='left' width=400 rowspan=2>" . getWords("note") . "</th>
        </tr>
        <tr>
          <th class=center width=40>" . getWords("yes") . "</th>
          <th class=center width=40>" . getWords("no") . "</th>
        </tr>
      </thead>
      <tbody>";
    for ($i = 1; $i <= 10; $i++) {
      ...
      <tr>
        ...
        <td align=center><label><input type=radio name=is_yes" . $i . " value='t' " . getCheckedFlagString(
                $arrData[$i]['is_yes'],
                true
            ) . " /></label></td>
        <td align=center><label><input type=radio name=is_yes" . $i . " value='f' " . getCheckedFlagString(
                $arrData[$i]['is_yes'],
                false
            ) . " /></td>";
    ...
    $strResult .= "
          <td nowrap colspan=3>
            <div class='col-md-12'><label class='col-md-4'>Besar Gaji: Rp.</label><div class='col-md-8'>" . generateInput("answer" . $i . "_1", $arrData[$i]['answer1'], "size=30") . "</div></div>
            <div class='col-md-12'><label class='col-md-4'>Fasilitas: </label><div class='col-md-8'>" . generateInput("answer" . $i . "_2", $arrData[$i]['answer2'], "size=30") . "</div></div>
          </td>
        </tr>";
    for ($i = 16; $i <= 17; $i++) {
        if (!isset($arrData[$i]['is_yes'])) {
    ...
    $strResult .= "
    </tbody></table>" . generateHidden("hNumShowQuestion", count($arrQuestion));
    return $strResult;
}
...
function getEmergency($strDataID)
{
    ...
    $strResult = "
      ...
      <thead>
        <tr>
          <th class=\"left\" width=250>" . getWords("name") . "</th>
          <th class=\"left\" width=250>" . getWords("address") . "</th>
          <th class=\"left\" width=120>" . getWords("phone") . "</th>
          <th class=\"left\" width=150>" . getWords("job") . " / " . getWords("position") . "</th>
          <th class=\"left\" width=150>" . getWords("relation") . "</th>
        </tr>
      <thead>
      <tbody>";
    $counter = 0;
...
function getJobReference($strDataID)
{
    ...
    $strResult = "";
    $strResult .= "<table class='table table-striped' border=0 cellpadding=1 cellspacing=0>";
    $counter = 1;
    ...
    foreach ($arrJobRef as $row) {
        ...
        if ($arrData['reference_type'] == $row['type']) {
            $strResult .= "<td width='140'><div class='radiobox'><label><input checked type=radio name=reference_type value=" . $row['type'] . " $strActionClick />" . getWords(
                    $row['name']
                ) . "</label></div></td>";
            ...
            $strResult .= "<td width='140'><div class='radiobox'><label><input type=radio name=reference_type value=" . $row['type'] . " $strActionClick />" . getWords(
                    $row['name']
                ) . "</label></div></td>";
            $strResult .= "<td width=10>:</td>";
            $strResult .= "<td>" . generateSelect(
    ...
    if ($arrData['reference_type'] == $counter) {
        $strResult .= "<td width='140'><div class='radiobox'><label><input checked type=radio name=reference_type value=" . $counter . "  />" . getWords(
                "other"
            ) . "</label></div></td>";
        $strResult .= "<td width=10>:</td>";
        $strResult .= "<td>" . generateInput("reference" . $counter, $arrData['reference'], "size=50") . "</td>";
    } else {
        $strResult .= "<td width='140'><div class='radiobox'><label><input type=radio name=reference_type value=" . $counter . "  />" . getWords(
                "other"
            ) . "</label></div></td>";
        $strResult .= "<td width=10>:</td>";
...
function getEducation($strDataID)
{
    ...
    $strResult = "
      <table class=\"table table-striped table-hover\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\">
        ...
        <tr>
          <th width=80>" . getWords("from") . "</th>
          <th width=80>" . getWords("to") . "</th>
        </tr>
...
function getWorkingExperience($strDataID)
{
    ...
    global $arrEmpty;
    $strResult = "
      <table class=\"table table-striped table-hover valign-top\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\">
      <thead>
        <tr>
          <th width=160>" . getWords("company information") . "</th>
          <th width=250>" . getWords("period") . "</th>
          <th width=200>" . getWords("position") . "</th>
		  <th width=200>" . getWords("location") . "</th>
		  <th width=200>" . getWords("job description") . "</th>
          <th width=170>" . getWords("other info") . "</th>
          <th width=200 class=left>" . getWords("reason for leaving") . "</th>
        </tr>
      </thead>
        ...
        if (is_array($arrResult)) {
            ...
            <tr name='dataWorkingExperienceRow$counter' id='dataWorkingExperienceRow$counter'  valign='top'>
              ...
              <td rowspan=2 nowrap>
                ...
                --><!--<strong>-->" . getWords("latest salary") . " : <!--</strong>--><br />" .
                    generateInput(
                        "lastSalary" . $counter,
                        $row['last_salary'],
            ...
            <tr name='dataWorkingExperienceRow2_$counter' id='dataWorkingExperienceRow2_$counter' valign='top' align='left'>
    ...
    if ($counter < $intMax) {
        for ($i = $counter; $i < $intMax; $i++) {
            ...
            $strResult .= "
              ...
              <td rowspan=2>
              <ol>
                <li>" . generateInput("reasonForLeaving1" . $counter, "", "size=25 tabIndex=" . ($tabIndex + 16)) . "</li>
                <li>" . generateInput("reasonForLeaving2" . $counter, "", "size=25 tabIndex=" . ($tabIndex + 16)) . "</li>
                <li>" . generateInput("reasonForLeaving3" . $counter, "", "size=25 tabIndex=" . ($tabIndex + 16)) . "</li>
              </ol>
              </td>
              " .//<!--<td align=center rowspan=2><a href=\"javascript:deleteWorkingExperience($counter)\" tabIndex=".($tabIndex+17)." title=\"".getWords("delete")."\"><img src=\"../images/delete.gif\" border=0 alt=\"".getWords("delete")."\" /></a></td>-->
                "</tr>
            <tr name='dataWorkingExperienceRow2_$counter' id='dataWorkingExperienceRow2_$counter'  $strStyle valign='top' class='left'>
              <td class='left'><strong>" . getWords("to") . "</strong><br />" .
                generateSelectDay(
                    "endDay" . $counter,
                    "",
                    "tabIndex=" . ($tabIndex + 5),
...
function saveData()
{
    ...
    if (saveDataCandidate()) {
        ...
        if (!empty($strDataID) && isset($arrData['dataPhoto']) && $arrData['dataPhoto'] != "") {
            // update data
            $db->connect();
...
function saveDataCandidate()
{
    ...
    ($data['application_date'] != "" || $data['application_date'] = NULL ) ? standardDateToSQLDateNew($data['application_date'], $_SESSION['sessionDateSetting']['date_sparator'], $_SESSION['sessionDateSetting']['pos_year'],$_SESSION['sessionDateSetting']['pos_month'], $_SESSION['sessionDateSetting']['pos_day'])
: NULL;
    ($data['candidate_entry_date'] != "" || $data['candidate_entry_date'] = NULL ) ? standardDateToSQLDateNew($data['candidate_entry_date'], $_SESSION['sessionDateSetting']['date_sparator'], $_SESSION['sessionDateSetting']['pos_year'],$_SESSION['sessionDateSetting']['pos_month'], $_SESSION['sessionDateSetting']['pos_day'])
: NULL;
($data['birthdate'] != "" || $data['birthdate'] = NULL ) ? standardDateToSQLDateNew($data['birthdate'], $_SESSION['sessionDateSetting']['date_sparator'], $_SESSION['sessionDateSetting']['pos_year'],$_SESSION['sessionDateSetting']['pos_month'], $_SESSION['sessionDateSetting']['pos_day'])
: NULL;
($data['idcard_expired_date'] != "" || $data['idcard_expired_date'] = NULL ) ? standardDateToSQLDateNew($data['idcard_expired_date'], $_SESSION['sessionDateSetting']['date_sparator'], $_SESSION['sessionDateSetting']['pos_year'],$_SESSION['sessionDateSetting']['pos_month'], $_SESSION['sessionDateSetting']['pos_day'])
: NULL;
($data['passport_expire_date'] != "" || $data['passport_expire_date'] = NULL ) ? standardDateToSQLDateNew($data['passport_expire_date'], $_SESSION['sessionDateSetting']['date_sparator'], $_SESSION['sessionDateSetting']['pos_year'],$_SESSION['sessionDateSetting']['pos_month'], $_SESSION['sessionDateSetting']['pos_day'])
: NULL;
($data['job_reference_date'] != "" || $data['job_reference_date'] = NULL ) ? standardDateToSQLDateNew($data['job_reference_date'], $_SESSION['sessionDateSetting']['date_sparator'], $_SESSION['sessionDateSetting']['pos_year'],$_SESSION['sessionDateSetting']['pos_month'], $_SESSION['sessionDateSetting']['pos_day'])
: NULL;
