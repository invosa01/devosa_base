...
$strFirstDate = getDateByInitialDay($strNow, 1, $strDateSparator, $strDatePosYear, $strDatePosMonth); //YYYY-MM-01
$strFirstDate = sqlToStandarDateNew(
    $strFirstDate,
    $strDateSparator,
    $strDateFormat
); // dirubah ke standar customer
$strDateFromView = '';
$strDateThruView = '';
$f = new clsForm(
    "form1", /*2 column view*/
...
$f->addInput(
    getWords("date from"),
    "dataDateFrom",
    ($strDateFrom = getInitialValue("DateFrom", $strFirstDate)),
...
$f->addInput(
    getWords("date thru"),
    "dataDateThru",
    ($strDateThru = getInitialValue("DateThru", $strNow)),
    ["style" => "width:$strDateWidth"],
    "date",
    false,
    true,
...
$strDivision = $f->getValue('dataDivision');
$strDepartemen = $f->getValue('dataDepartment');
if ($showReport) {
    ...
    if ($strDivision != "") {
        if ($strDivision != "C0100") { // khusus direksi tidak melihat divisi
            $strKriteria .= "AND t0.division_code = '" . $strDivision . "' ";
        }
    }
    if ($strDepartemen != "") {
        $strKriteria .= "AND department_code = '" . $strDepartemen . "' ";
    }
    $strErrorMessage = "";
    ...
    $myDataGrid->addSpecialButton(
        ...
        "btnSlip",
        "submit",
        getWords("get attendance slip"),
        "onClick=\"document.form1.target = '_blank'\"",
        "getSlip()"
    );
...
$strPageDesc = getWords('Attendance Slip Report');
...
function getSlip()
{
    ...
    //$intDateFrom = mktime(0, 0, 0, $splitDateFrom[0], $splitDateFrom[1], $splitDateFrom[2]);
    //$splitDateThru = explode('/', $strDateThru);
    //$strDateThru = $splitDateThru[2] . '-' . $splitDateThru[0] . '-' . $splitDateThru[1];
    $strDateThruView = $strDateThru;
    ...
    //$intDateThru = mktime(0, 0, 0, $splitDateThru[0], $splitDateThru[1], $splitDateThru[2]);
    //$strDateThruView = date('d F Y', $intDateThru);
    $dateArray = createDateRangeArray($strDateFrom, $strDateThru);
...
<link rel='stylesheet' href='../asset/scripts/vendor/bootstrap/dist/css/bootstrap.min.css'>
</head>
...
<table cellspacing=0 cellpadding=0 border=0 width='100%'>
    ...
    foreach ($myDataGrid->checkboxes as $strValue) {
        ...
        $strEmployeeID = $objEmp->getInfoByID($strIDEmployee, "employee_id");
        $strDiv = $objEmp->getInfoByID($strIDEmployee, "division_code");
        $strDept = $objEmp->getInfoByID($strIDEmployee, "department_code");
        $strSect = $objEmp->getInfoByID($strIDEmployee, "section_code");
        ...
        $GLOBALS['strSection'] = getSectionName($strSect);
        ...
        while ($row = $db->fetchrow($res)) {
            $arrResult[$row['attendance_date']]['attendance_date'] = date("d M Y", strtotime($row['attendance_date']));
            ...
            //$arrResult[$row['attendance_date']]['late_duration'] = ($row['late_duration'] <= 15) ? 0 : $row['late_duration'];
            $arrResult[$row['attendance_date']]['late_duration'] = ($row['late_duration'] <= 0) ? 0 : $row['late_duration'];
            $arrResult[$row['attendance_date']]['early_duration'] = ($row['early_duration'] <= 0) ? 0 : $row['early_duration'];
            $arrResult[$row['attendance_date']]['attendance_code'] = 'H';
        }
        //$strSQL = "SELECT * FROM hrd_absence_detail WHERE id_employee = $strIDEmployee AND status='2' AND absence_date BETWEEN '$strDateFrom' AND '$strDateThru'";
        $strSQL = "SELECT t1.*,t2.status FROM hrd_absence_detail as t1, hrd_absence as t2 WHERE t1.id_employee = $strIDEmployee AND t2.status='2' AND t1.id_absence=t2.id AND t1.absence_date BETWEEN '$strDateFrom' AND '$strDateThru'";
        $res = $db->execute($strSQL);
        while ($row = $db->fetchrow($res)) {
            $arrResult[$row['absence_date']]['attendance_date'] = date("d M Y", strtotime($row['absence_date']));
        ...
        $totalLate = 0;
        $totalEarly = 0;
        foreach ($dateArray as $strDate) {
            // echo 'a';die();
            $rowCSS = '';
            //#F6CED8
            if (isset($arrResult[$strDate])) {
                $strInfo = $arrResult[$strDate];
                $explodeDate = explode('-', $strDate);
                $intDate = mktime(0, 0, 0, $explodeDate[1], $explodeDate[2], $explodeDate[0]);
                $isHoliday = isHoliday($strDate);
                if (date('w', $intDate) == 6) {
                    $rowCSS = ' class="sat-back"';
                } else if (date('w', $intDate) == 0) {
                    $rowCSS = ' class="sun-back"';
                } else if ($isHoliday) {
                    $strInfo['attendance_code'] = '-';
                    $rowCSS = ' class="hol-back"';
                }
            } else {
                $explodeDate = explode('-', $strDate);
                $isHoliday = isHoliday($strDate, false);
                $intDate = mktime(0, 0, 0, $explodeDate[1], $explodeDate[2], $explodeDate[0]);
                $strDateView = date('d M Y', $intDate);
                $strInfo['attendance_date'] = $strDateView;
                $strInfo['attendance_start'] = '-';
                $strInfo['attendance_date'] = $strDateView;
                $strInfo['attendance_finish'] = '-';
                $strInfo['late_duration'] = 0;
                $strInfo['early_duration'] = 0;
                if (date('w', $intDate) != 0 && date('w', $intDate) != 6 && !$isHoliday) {
                    $strInfo['attendance_code'] = 'A';
                } else if (!$isHoliday) {
                    $strInfo['attendance_code'] = '-';
                    $rowCSS = ' class="nor-back"';
                    if (date('w', $intDate) == 6) {
                        $rowCSS = ' class="sat-back"';
                    } else if (date('w', $intDate) == 0) {
                        $rowCSS = ' class="sun-back"';
            ...
            $GLOBALS['strResult'] .= "<td style='text-align: right;padding: 4px;' " . $rowCSS . ">" . $counter . "</td>";
            $GLOBALS['strResult'] .= "<td style='text-align: center;padding: 4px;' " . $rowCSS . ">" . $strInfo['attendance_date'] . "</td>";
            $GLOBALS['strResult'] .= "<td style='text-align: center;padding: 4px;' " . $rowCSS . ">" . $strInfo['attendance_start'] . "</td>";
            $GLOBALS['strResult'] .= "<td style='text-align: center;padding: 4px;' " . $rowCSS . ">" . $strInfo['attendance_code'] . "</td>";
            //$GLOBALS['strResult'] .= "<td style='text-align: center;padding: 4px;' ".$rowCSS.">1</td>";
            $GLOBALS['strResult'] .= "<td style='text-align: center;padding: 4px;' " . $rowCSS . ">" . $strInfo['attendance_date'] . "</td>";
            $GLOBALS['strResult'] .= "<td style='text-align: center;padding: 4px;' " . $rowCSS . ">" . $strInfo['attendance_finish'] . "</td>";
            $GLOBALS['strResult'] .= "<td style='text-align: right;padding: 4px;' " . $rowCSS . ">" . $strInfo['late_duration'] . "</td>";
            $totalLate = $totalLate + $strInfo['late_duration'];
            $GLOBALS['strResult'] .= "<td style='text-align: right;padding: 4px;' " . $rowCSS . ">" . $strInfo['early_duration'] . "</td>";
            $totalEarly = $totalEarly + $strInfo['early_duration'];
            //$GLOBALS['strResult'] .= "<td style='text-align: right;padding: 4px;' ".$rowCSS.">0</td>";
            $GLOBALS['strResult'] .= "</tr>";
            $counter++;
        }
        $GLOBALS['strResult'] .= "<tr>";
        $GLOBALS['strResult'] .= "<td colspan='6' style='text-align: right;padding: 4px;font-size: 11px;'><strong>Total</strong></td>";
        $GLOBALS['strResult'] .= "<td style='text-align: right;padding: 4px;font-size: 11px;'><strong>$totalLate</strong></td>";
        $GLOBALS['strResult'] .= "<td style='text-align: right;padding: 4px;font-size: 11px;'><strong>$totalEarly</strong></td>";
        //$GLOBALS['strResult'] .= "<td style='text-align: right;padding: 4px;font-size: 11px;'><strong>0</strong></td>";
        $GLOBALS['strResult'] .= "</tr>";
        if ($bolEven) // genap
        {
            echo "<br><br><br>";
            echo "<p>&nbsp;</p><p>&nbsp;</p><table><tr><td height=30>&nbsp;</td></tr></table><span>";
            echo $strNewPage;
        } else if ($i == 1) {
            echo $strNewPage;
