...
/* function standardDateToSQLDateNew
 * fungsi untuk mengubah tanggal dari format CUSTOMER ke format YYYY-DD-MM
 * $dateSparator : string ( -, / )
 * $arrYear, $arrMonth, $arrDate : integer ( posisi tahun, bulan dan tanggal dari tanggal yang diinput )
 * Example : Tanggal standart customer DD-MM-YYYY $posArrYear = 2, $posArrMonth = 1 $posArrDay = 0
 * Example : Tanggal standart customer MM/DD/YYYY $posArrYear = 2, $posArrMonth = 0 $posArrDay = 1
 * Sanusi : 02 Agustus 2016
 */
function standardDateToSQLDateNew($strDate, $dateSparator = "/", $posArrYear = 2, $posArrMonth = 1, $posArrDay = 0)
{
    $strResult = $strDate;
    if ($strDate != "") {
        $arrDate = explode($dateSparator, $strDate);
        if (count($arrDate) >= 3) {
            $strResult = $arrDate[$posArrYear] . "-" . $arrDate[$posArrMonth] . "-" . $arrDate[$posArrDay];
        }
    }
    return $strResult;
}

/* function sqlToStandarDeteNew()
 * fungsi untuk mengubah tanggal dari postgressql "YYYY-MM-DD" ke format CUSTOMER
 * $dateFormat : Php Formated Date pola (d,j) day, ( m, M, n, F ) Month, ( y, Y) Year
 * Example for Input : Customer date 31-12-2016 , $date Format = "dmY" $dateSparator = "-"
 * Example :  Postgres : 2016-12-31 Output : 31-12-2016,
 * Sanusi : 02 Agustus 2016
 * Last Revision : 03 Agustus 2016
 */
function sqlToStandarDateNew($strDate, $dateSparator = "/", $dateFormat = "dmY")
{
    $strResult = "";
    if ($strDate != "") {
        list($intTahun, $intBulan, $intTanggal) = explode("-", $strDate);
        $intTahun = (int)$intTahun;
        $intTanggal = (int)$intTanggal;
        $intBulan = (int)$intBulan;
        // loop per karakter
        $intLength = strlen($dateFormat);
        for ($i = 0; $i < $intLength; $i++) {
            if ($dateFormat[$i] == 'd') { // tanggal 2 digit
                $strResult .= ($intTanggal < 10) ? "0" . $intTanggal : $intTanggal;
            } else if ($dateFormat[$i] == 'j') { // tanggal tidak harus 2 digit
                $strResult .= (int)$intTanggal;
            } else if ($dateFormat[$i] == 'm') { // bulan, angka 2 digit
                $strResult .= ($intBulan < 10) ? "0" . $intBulan : $intBulan;
            } else if ($dateFormat[$i] == 'M') { // bulan, 3 huruf
                $strResult .= getBulanSingkat((int)$intBulan);
            } else if ($dateFormat[$i] == 'n') { // bulan, angka tidak harus 2 digit
                $strResult .= (int)$intBulan;
            } else if ($dateFormat[$i] == 'F') { // bulan, lengkap
                $strResult .= getBulan((int)$intBulan);
            } else if ($dateFormat[$i] == 'y') { // tahun 2 digit
                $strResult .= substr($intTahun, 2, 2);
            } else if ($dateFormat[$i] == 'Y') { // tahun, angka 4 digit
                $strResult .= $intTahun;
            }
            if ($i < $intLength - 1) {
                $strResult .= $dateSparator;
            }
        }
    }
    return $strResult;
...
/* function validStandardDateNew
* fungsi untuk cek validasi tanggal, return : bolean
* $dateSparator : string ( -, / )
* $arrYear, $arrMonth, $arrDate : integer ( posisi tahun, bulan dan tanggal dari tanggal yang diinput )
* Example : Tanggal standart customer DD-MM-YYYY $posArrYear = 2, $posArrMonth = 1 $posArrDate = 0
* Example : Tanggal standart customer MM/DD/YYYY $posArrYear = 2, $posArrMonth = 0 $posArrDate = 1
* Sanusi : 02 Agustus 2016
*/
function validStandardDateNew($strDate, $dateSparator = "/", $posArrYear = 2, $posArrMonth = 1, $posArrDay = 0)
{
    $bolResult = false;
    if ($strDate != "") {
        $arrDate = explode($dateSparator, $strDate);
        $year = (int)$arrDate[$posArrYear];
        $month = (int)$arrDate[$posArrMonth];
        $day = (int)$arrDate[$posArrDay];
        $bolResult = checkdate($month, $day, $year);
    }
    return $bolResult;
}

/* function getDateByInitialDay
* fungsi untuk membuat tanggal ke sql dengan default tanggal yang di set.
 * $initialDay  = 1 s/d akhir bulan berjalan
* $dateSparator : string ( -, / )
* $arrYear, $arrMonth, $arrDate : integer ( posisi tahun, bulan dan tanggal dari tanggal yang diinput )
* Example : Tanggal standart customer DD-MM-YYYY $posArrYear = 2, $posArrMonth = 1 $posArrDate = 0
* Example : Tanggal standart customer MM/DD/YYYY $posArrYear = 2, $posArrMonth = 0 $posArrDate = 1
* Sanusi : 15 November 2016
*/
function getDateByInitialDay($strDate, $initialDay, $dateSparator = "/", $posArrYear = 2, $posArrMonth = 1)
{
    //Ex Today : 15/11/2016 , $initialDay = 5 , $initialDay = 1
    $strResult = $strDate;
    if ($strDate != "") {
        $arrDate = explode($dateSparator, $strDate);
        if (count($arrDate) >= 3) {
            $strResult = $arrDate[$posArrYear] . "-" . $arrDate[$posArrMonth] . "-" . $initialDay;
        }
    }
    return $strResult; // 2106-11-05 , 2106-11-01
}
